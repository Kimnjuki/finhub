...
export const createAlert = action({
    async handler(ctx, { userId, instrumentId, type, conditionConfig, deliveryChannels, cooldownSeconds, expiresAt }) {
...
export const getAlerts = query({
    async handler(ctx, { userId, instrumentId, limit = 50 }) {
        const identity = await ctx.auth.getUserIdentity();
        if (!identity)
            throw new Error("Unauthenticated");
        const access = await checkAccess(ctx, identity.subject, "alerts");
        if (!access.allowed)
            throw new Error(access.reason);
        let query = ctx.db.query("alerts").filter((q) => q.eq("userId", userId));
        if (instrumentId) {
            query = query.filter((q) => q.eq("instrumentId", instrumentId));
        }
