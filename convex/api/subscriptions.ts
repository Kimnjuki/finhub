...
export const seedPlans = mutation({
  ...
  handler: async (ctx: MutationCtx) => {
    ...
    if (existingPlans) return { seeded: false, reason: "Plans already exist" };
...
export const getUserSubscription = query({
  ...
  handler: async (ctx: QueryCtx, { userId }: { userId: string }) => {
    ...
    const plan = await ctx.db.query("subscriptionPlans")
      .withIndex("by_name", (q: any) => q.eq("name", sub.planId === "free" ? "Free" : sub.planId))
      .first();
...
export const subscribeToPlan = mutation({
  ...
  handler: async (ctx: MutationCtx, args: { userId: string; planName: string; billingCycle: "monthly" | "yearly" }) => {
    ...
    const plan = await ctx.db
      .query("subscriptionPlans")
      .withIndex("by_name", (q: any) => q.eq("name", planName))
      .first();
