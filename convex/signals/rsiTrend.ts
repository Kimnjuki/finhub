...
// Interface for RSI trend signal arguments
interface RsiTrendArgs {
  instrumentId: string;
  timeframe?: string;
  rsiPeriod?: number;
}
...
export const checkRsiTrend = convex.defineAction({
  async handler(ctx: any, args: RsiTrendArgs) {
    ...
    const ohlcvData = await ctx.db.query("ohlcvData")
      .withIndex("by_instrument_interval", (q: any) => q.eq("instrumentId", instrumentId).eq("interval", timeframe))
      .take(50)
      .collect();
    ...
    ohlcvData.sort((a: OhlcvData, b: OhlcvData) => a.tsUtc - b.tsUtc);
...
function calculateRSI(ohlcvData: OhlcvData[], period: number): number[] {
