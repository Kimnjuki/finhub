...
export const updateStreamMetrics = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const stream = await ctx.db
            .query("marketStreams")
            .withIndex("by_instrument_and_source", (q) => q.eq("instrumentId", args.instrumentId).eq("sourceId", args.sourceId))
            .first();
        if (stream) {
        ...
        const existingHealth = await ctx.db
            .query("streamHealth")
            .withIndex("by_instrument_channel", (q) => q.eq("instrumentId", args.instrumentId).eq("channel", stream?.channel || 'trades'))
            .first();
        if (existingHealth) {
