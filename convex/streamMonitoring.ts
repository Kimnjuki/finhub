...
export interface StreamHealth {
  status: 'active' | 'stale' | 'resyncing' | 'disabled';
  avgLatencyMs: number;
  latencyP99Ms: number;
...
export function monitorStream(metrics: {
  messagesReceived: number;
  lastMessageAt: number;
  latencyMs: number[];
  errorCount: number;
  reconnectAttempts: number;
}, now = Date.now()): StreamHealth {
  ...
  let status: StreamHealth['status'] = 'active';
...
export function shouldAlert(health: StreamHealth): boolean {
  return health.avgLatencyMs > 200 || health.lastMessageAge > 30000 || health.errorRate > 0.1;
}

export function getHealthScore(health: StreamHealth): number {
  let score = 100;
...
export function getPerformanceTier(avgLatencyMs: number): string {
  if (avgLatencyMs < 50) return 'HFT (Sub-50ms)';
  if (avgLatencyMs < 200) return 'Enterprise (50-200ms)';
  if (avgLatencyMs < 500) return 'Standard (200-500ms)';
...
export function formatMessageRate(mps: number): string {
  if (mps < 1000) return `${mps.toFixed(1)}/s`;
...
export function generateDiagnosticReport(
  streams: Map<string, { metrics: ReturnType<typeof monitorStream> }>
): DiagnosticReport {
  ...
  const alerts: DiagnosticReport['alerts'] = [];
