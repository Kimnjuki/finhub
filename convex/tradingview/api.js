...
export const createWebhookConfig = actionGeneric({
    ...
    async handler(ctx, args) {
        ...
        const hmacSecret = Array.from({ length: 32 }, () => Math.random().toString(36).charAt(2)).join('');
...
export const regenerateHmacSecret = actionGeneric({
    ...
    async handler(ctx, args) {
        const newSecret = Array.from({ length: 32 }, () => Math.random().toString(36).charAt(2)).join('');
...
export const saveExchangeCredentials = actionGeneric({
    ...
    async handler(ctx, args) {
        ...
        const existing = await ctx.db.query("exchangeCredentials")
            .withIndex("by_user_and_exchange", (q) => q.eq("userId", args.userId).eq("exchange", args.exchange))
        ...
        const credentialId = await ctx.db.insert("exchangeCredentials", {
            ...
            permissions: (args.permissions || ['read', 'trade']),
            createdAt: now,
            updatedAt: now,
        });
        return { credentialId };
...
export const listWebhookEvents = actionGeneric({
    ...
    async handler(ctx, args) {
        ...
        const events = await ctx.db.query("tvWebhookEvents")
            .withIndex("by_user_and_received", (q) => q.eq("userId", args.userId))
            .order("desc")
            .take(limit)
            .collect();
        return events;
...
export const listOpenPositions = actionGeneric({
    ...
    async handler(ctx, args) {
        const positions = await ctx.db.query("tvPositions")
            .withIndex("by_user_and_open", (q) => q.eq("userId", args.userId).eq("isOpen", true))
            .collect();
        return positions;
    },
});
