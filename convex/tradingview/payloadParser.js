 ...
 */
export function parseTradingViewAlert(rawPayload) {
    try {
        const payload = JSON.parse(rawPayload);
        const rawFields = payload;
        ...
        else if (typeof payload.order_contracts === 'number')
            quantity = payload.order_contracts;
        else if (typeof payload.position_size === 'number')
            quantity = Math.abs(payload.position_size);
        else if (payload.strategy?.order_contracts)
            quantity = payload.strategy.order_contracts;
        else if (payload.strategy?.position_size)
            quantity = Math.abs(payload.strategy.position_size);
        else if (payload.volume)
            quantity = Math.abs(payload.volume);
        ...
        if (payload.stop_price || payload.stop)
            orderType = 'stop';
        const strategy = payload.strategy_name ||
        ...
        if (!normalizedSide) {
            ...
            if (payload.strategy?.market_position === 'long')
                normalizedSide = 'buy';
            else if (payload.strategy?.market_position === 'short')
                normalizedSide = 'sell';
            else if (payload.strategy?.market_position === 'flat') {
                // Closing position - infer from position_size direction
                if (payload.position_size && payload.position_size > 0)
                    normalizedSide = 'sell';
                else if (payload.position_size && payload.position_size < 0)
                    normalizedSide = 'buy';
            }
        }
        if (!normalizedSide) {
...
function normalizeSide(side) {
    ...
    if (buyTerms.includes(lower))
        return 'buy';
    if (sellTerms.includes(lower))
        return 'sell';
    if (shortTerms.includes(lower))
        return 'short';
    if (coverTerms.includes(lower))
        return 'cover';
    // Check for partial matches
    if (lower.includes('buy') || lower.includes('long') || lower === 'bull')
        return 'buy';
    if (lower.includes('short') && !lower.includes('cover') && !lower.includes('close'))
        return 'short';
    if (lower.includes('cover') || (lower.includes('close') && lower.includes('short')))
        return 'cover';
    if (lower.includes('sell') || lower.includes('bear') || lower === 'exit' || lower === 'close')
        return 'sell';
    return undefined;
}
